/**
 * PEM-encoded certificate chain for the OpenHarmony Application Release key.
 *
 * Root-CA → Application-CA → Application-Release leaf. This is the public
 * OpenHarmony test PKI that ships with system app source trees (e.g.
 * `applications/standard/launcher/signature/OpenHarmonyApplication.cer`). It
 * pairs with the private key stored under alias `OpenHarmony Application Release`
 * in the SDK's `OpenHarmony.p12` keystore.
 *
 * Used to sign HAPs that request privileged permissions (apl >= system_basic).
 * For apl=normal the existing `OpenHarmonyProfileRelease.pem` chain — already
 * shipped in the SDK — is used instead.
 */
export const OPEN_HARMONY_APPLICATION_CERT = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

zAlF08DnbJrOOtOnQq5wHOPlDYB4OtUzOYJk9scotrEnJxJzGsh/
-----END CERTIFICATE-----
`;
